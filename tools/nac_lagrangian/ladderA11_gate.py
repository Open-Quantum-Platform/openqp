...
def main():
    ...
    print('\nsolving interstate z-vectors via the seam (BOTH targets)...', flush=True)

    def seam_solve(target, Lm):
    ...
    for I in range(nstate):
        for J in range(I + 1, nstate):
            ...
            resp_z[(I, J)] = seam_solve(I + 1, Lm)
            resp_zT[(I, J)] = seam_solve(J + 1, Lm)
            n1 = 0.0 if resp_z[(I, J)] is None else np.linalg.norm(resp_z[(I, J)])
            n2 = 0.0 if resp_zT[(I, J)] is None else np.linalg.norm(resp_zT[(I, J)])
            print(f'  ({I+1},{J+1}): |zB(tgt=I)|={n1:.6f}  |zB(tgt=J)|={n2:.6f}', flush=True)
    ...
    print('\n===== LADDER A11: seam target-ordering + conditioning =====')
    for I in range(nstate):
        for J in range(nstate):
            if I >= J:
                continue
            ...
            es = esum_v[(I + 1, J + 1)]
            resid = sc - (g1 + es) / gap
            print(f'pair ({I+1},{J+1}): |resid|={np.linalg.norm(resid):.6f} gap={gap:.6f}')
            for nm, zb in (('tgt=I', resp_z.get((I, J))),
                           ('tgt=J', resp_zT.get((I, J)))):
                if zb is None:
                    print(f'    {nm}: no solution')
                    continue
                for s in (1.0, -1.0):
                    v = s * zb / gap
                    ...
                    print(f'    {nm} s={s:+.0f}: |v|={np.linalg.norm(v):.6f} '
