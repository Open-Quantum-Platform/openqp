"""v7j: THE ONE-PROCESS UNIFIED GATE. Mt harvest, direct-injection
...
def main():
    ...
    r = Runner(input_file=inp, log=inp.replace('.inp', '_v7o.log'))
    r.run()
    mol = r.mol
    ...
    # ---- second sweep at h/2 + J1 h-scaling -------------------------------
    Ux1 = Ux_FD.copy()
    w1 = w_ref.copy()
    HD2 = 5.0e-4
    print('second displaced sweep h/2...', flush=True)
    Ux2 = np.zeros((ncoord, nbf, nbf))
    w2 = np.zeros((ncoord, nstate, nij))
    for c in range(ncoord):
        cp = xyz0.copy(); cp[c] += HD2
        Mp, Skp, Axp = displaced_all(cp)
        cm = xyz0.copy(); cm[c] -= HD2
        Mm, Skm, Axm = displaced_all(cm)
        Ux2[c] = (Mp - Mm) / (2 * HD2) - (Skp - Skm) / (2 * HD2)
        w2[c] = (Axp - Axm) / (2 * HD2)
    mol.update_system(xyz0)
    oqp.library.ints_1e(mol)
    for k, v in SAVE0.items():
    ...
    for I in range(nstate):
        for J in range(nstate):
            ...
            for tag, U, w in (('h   ', Ux1, w1), ('h/2 ', Ux2, w2),
                              ('Rich', UxR, wR)):
                lhs = np.array([float(np.sum(Mfull * U[c]))
                                for c in range(ncoord)])
                rhs = np.array([float(np.dot(ytil[(I, J)], w[c, J]))
                                for c in range(ncoord)]) - T1[(I, J)]
                dv = lhs - rhs
                print(f'({I+1},{J+1})[{tag}]: deficit |d|={np.linalg.norm(dv):.5f} '
                      f'maxdiff={np.abs(dv).max():.3e}')
    np.savez(inp.replace('.inp', '_v7o.npz'), Ux1=Ux1, Ux2=Ux2, w1=w1, w2=w2,
             **{f'MT_{I}{J}': MT[(I, J)] for (I, J) in MT},
             **{f'MTG_{I}{J}': MTG[(I, J)] for (I, J) in MTG},
             **{f'T1_{I}{J}': T1[(I, J)] for (I, J) in T1},
             **{f'T2d_{I}{J}': T2d[(I, J)] for (I, J) in T2d},
             **{f'ytil_{I}{J}': ytil[(I, J)] for (I, J) in ytil})
    print('saved v7o npz.', flush=True)
