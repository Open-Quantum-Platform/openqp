"""v7f: the CALIBRATED bookkeeping (v7e unit-L seam calibration):
...
Run: python v7f_h2o.py <input.inp> <dnum.npz>
"""
import os
import sys
import numpy as np
...
def main():
    ...
    r = Runner(input_file=inp, log=inp.replace('.inp', '_v7f.log'))
    r.run()
    mol = r.mol
    ...
    # ---- assemble ----------------------------------------------------------
    print('\n===== v7f ASSEMBLY (calibrated bookkeeping) vs d_num =====')
    ...
    np.savez(inp.replace('.inp', '_v7f.npz'), dp=dp, dpa=dpa)
