...
def main():
    ...
    for I in range(nstate):
        for J in range(nstate):
            ...
            es = esum_v[(I + 1, J + 1)]
            resid = sc - (g1 + es) / gap
            LU = np.array([float(np.dot(L_IJ[(I, J)], Ux_pack[k]))
                           for k in range(ncoord)])
            for s, nm in ((1.0, '+LU/gap'), (-1.0, '-LU/gap'),
                          (0.5, '+LU/2gap'), (-0.5, '-LU/2gap')):
                v = s * LU / gap
                c = float(np.dot(resid, v)) / (np.linalg.norm(resid)
                                               * np.linalg.norm(v) + 1e-300)
                print(f'  ({I+1},{J+1}) {nm:10s}: |resid|={np.linalg.norm(resid):.6f} '
