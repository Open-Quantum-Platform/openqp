...
def main():
    ...
    r = Runner(input_file=inp, log=inp.replace('.inp', '_v10.log'))
    r.run()
    mol = r.mol
    ...
    def set_maxit(n):
        for attr in ('maxit', 'scf_maxit'):
            try:
                setattr(mol.data._data.control, attr, n)
                return
            except Exception:
                continue
    ...
    # worker-style skel: 1-iteration SCF => F(x')[D_ref]
    mol.update_system(xyz0); oqp.library.ints_1e(mol)
    for k, v in SAVE0.items():
        mol.data[k] = v.copy()
    set_maxit(1)
    mol.config['scf']['maxit'] = 1
    ws1 = (disp(cp, True) - disp(cm, True)) / (2 * HD)
    set_maxit(30)
    mol.config['scf']['maxit'] = 30
    mol.update_system(xyz0); oqp.library.ints_1e(mol)
    for k, v in SAVE0.items():
        mol.data[k] = v.copy()
    trueG = ws - ws1        # F-channel: [A(C0,F[D'])-A(C0,F[D_ref])] FD
    # h/2 C-channel
    HD2 = 5e-4
    cp2 = xyz0.copy(); cp2[c0] += HD2
    cm2 = xyz0.copy(); cm2[c0] -= HD2
    wr2 = (disp(cp2, False) - disp(cm2, False)) / (2 * HD2)
    mol.update_system(xyz0); oqp.library.ints_1e(mol)
    for k, v in SAVE0.items():
        mol.data[k] = v.copy()
    ws2 = (disp(cp2, True) - disp(cm2, True)) / (2 * HD2)
    mol.update_system(xyz0); oqp.library.ints_1e(mol)
    for k, v in SAVE0.items():
        mol.data[k] = v.copy()
    mol.update_system(xyz0); oqp.library.ints_1e(mol)
    ...
    # C-channel-pure comparison: lhs (converged-F both branches) vs staged only
    lhs2 = wr2 - ws2
    diffC = lhs - staged
    print(f'c0={c0} J={J+1}: |lhs(C-chan)|={np.linalg.norm(lhs):.5f} '
          f'|staged|={np.linalg.norm(staged):.5f} |gvec|={np.linalg.norm(gvec):.5f}')
    print(f'C-channel: |lhs - staged| = {np.linalg.norm(diffC):.5f} '
          f'maxdiff={np.abs(diffC).max():.4e}')
    print(f'(old miswired: |lhs - staged - gvec| = '
          f'{np.linalg.norm(lhs - staged - gvec):.5f})')
    diffC2 = lhs2 - staged
    print(f'C-channel h/2: |lhs2 - staged| = {np.linalg.norm(diffC2):.5f} '
          f'(ratio to h: {np.linalg.norm(diffC2)/max(np.linalg.norm(diffC),1e-30):.3f})')
    dg = trueG - gvec
    print(f'TRUE-G vs gvec: |trueG|={np.linalg.norm(trueG):.5f} '
          f'|gvec|={np.linalg.norm(gvec):.5f} |diff|={np.linalg.norm(dg):.5f} '
          f'maxdiff={np.abs(dg).max():.4e}')
    diff = diffC
