"""v8: w-VECTOR-level localization of Delta. One coordinate c0:
...
def main():
    ...
    r = Runner(input_file=inp, log=inp.replace('.inp', '_v8.log'))
    r.run()
    mol = r.mol
    ...
    def disp(coord, frozen):
        ...
        try:
            ...
            pass
        if frozen:
            # keep displaced FOCK(=F(x')[D..]) but frozen C
            mol.data['OQP::VEC_MO_A'] = W0
            mol.data['OQP::VEC_MO_B'] = Wb0
            return matvec(XJ)
        else:
            out = sg_apply(matvec(sg_apply(XJ, sg)), sg)
            return out
