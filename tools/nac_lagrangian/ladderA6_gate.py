...
def main():
    ...
    print('\n============ LADDER A6: z-vector response vs frozen residuals =====')
    for I in range(nstate):
        for J in range(nstate):
            if I >= J:
                continue
            ...
            es = esum_v[(I + 1, J + 1)]
            resid = sc - (g1 + es) / gap
            if resp_z.get((I, J)) is None:
                print(f'pair ({I+1},{J+1}): no z solution')
                continue
            zb = resp_z[(I, J)]
            for s, nm in ((1.0, '+zB/gap'), (-1.0, '-zB/gap')):
                v = s * zb / gap
                c = float(np.dot(resid, v)) / (np.linalg.norm(resid)
                                               * np.linalg.norm(v) + 1e-300)
                print(f'  ({I+1},{J+1}) {nm:9s}: |resid|={np.linalg.norm(resid):.6f} '
                      f'|v|={np.linalg.norm(v):.6f} cos={c:+.6f} '
