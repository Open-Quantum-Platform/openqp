    ...
    for I in range(nstate):
        for J in range(nstate):
            ...
            for k in range(ncoord):
                ...
                Ua = 0.5 * (U - U.T)
                Us = 0.5 * (U + U.T)
                LUa[k] = float(np.sum(La[I, J] * Ua))
                LUs[k] = float(np.sum(Ls[I, J] * Us))
                for bn, (lo, hi) in blocks.items():
                    m = np.zeros((nbf, nbf))
                    m[lo, hi] = 1.0
                    m[hi, lo] = 1.0
                    per_block.setdefault(bn, np.zeros(ncoord))[k] = float(
                        np.sum((La[I, J] * Ua + Ls[I, J] * Us) * m))
            print(f'pair ({I+1},{J+1}): |resid|={np.linalg.norm(resid):.6f}')
            for nm, v in (('-(LUa)/gap', -LUa / gap),
                          ('-(LUa+LUs)/gap', -(LUa + LUs) / gap),
                          ('+(LUa+LUs)/gap', (LUa + LUs) / gap)):
                c = float(np.dot(resid, v)) / (np.linalg.norm(resid)
                                               * np.linalg.norm(v) + 1e-300)
                print(f'    {nm:16s} |v|={np.linalg.norm(v):.6f} cos={c:+.6f} '
                      f'closure={np.linalg.norm(resid - v):.6f}')
            for bn in blocks:
                print(f'      block {bn}: |contrib/gap|='
                      f'{np.linalg.norm(per_block[bn] / gap):.6f}')
