...
def main():
    ...
    # ---- ADJOINT (in-process, phase-consistent) --------------------------
    ...
    for I in range(nstate):
        for J in range(nstate):
            ...
            for c in range(ncoord):
                ...
                rhs = float(np.dot(G_met[I, J], dX_PTref[c, J]))
                aid = max(aid, abs(lhs - rhs))
    print(f'ADJOINT IDENTITY: max|ytil.w - G_met.PT(w)| = {aid:.3e}', flush=True)
    damp = np.zeros((ncoord, nstate, nstate))
    dorb = np.zeros((ncoord, nstate, nstate))
    for I in range(nstate):
        for J in range(nstate):
            ...
            for c in range(ncoord):
                ...
                dorb[c, I, J] = float(np.sum(gam[I, J] * (Sk_an[c] + Ux_FD[c])))
    dp = damp + dorb
    dpa = 0.5 * (dp - dp.transpose(0, 2, 1))
    print('===== W6: ADJOINT-FORM d vs d_num (in-process) =====')
    for I in range(nstate):
        for J in range(nstate):
            if I >= J:
                continue
            ...
            print(f'pair ({I+1},{J+1}): |d_num|={np.linalg.norm(dn):.6f} '
                  f'|pred|={np.linalg.norm(v):.6f} cos={cc:+.8f} '
                  f'maxdiff={np.abs(v-dn).max():.3e}')
    np.savez(inp.replace('.inp', '_v3h.npz'), w_ref=w_ref, Ux_FD=Ux_FD,
             Sk_an=Sk_an, G_met=G_met, ytil=ytil, Xf=Xf, w_full=w_full)
