...
def main():
    ...
    # -------- branch's validated transported-matvec damp (existing code) ----
    print('running _compute_amp_damp (branch transported matvec)...', flush=True)
    ...
    nacobj = NAC(mol)
    damp = nacobj._compute_amp_damp(dx=1.0e-3)
    print('  damp done:', 'OK' if damp is not None else 'FAILED', flush=True)

    print('\n===== FINAL: branch transported damp vs scaffold amp (/gap) =====')
    for I in range(nstate):
        for J in range(nstate):
            if I >= J:
                continue
            gap = Om[J] - Om[I]
            sc = d_amp[:, I, J] / gap          # scaffold amp term /gap
            if damp is None or damp.get((I + 1, J + 1)) is None:
                print(f'pair ({I+1},{J+1}): branch damp unavailable')
                continue
            bd = damp[(I + 1, J + 1)]
            cos = float(np.dot(sc, bd)) / (np.linalg.norm(sc)
                                           * np.linalg.norm(bd) + 1e-300)
            print(f'pair ({I+1},{J+1}): |scaffold/gap|={np.linalg.norm(sc):.6f}  '
                  f'|branch damp|={np.linalg.norm(bd):.6f}  cos={cos:+.6f}  '
                  f'max|diff|={np.abs(sc-bd).max():.2e}')
            print(f'  gauge-difference (transport term) |sc-bd| = '
                  f'{np.linalg.norm(sc-bd):.6f}')
