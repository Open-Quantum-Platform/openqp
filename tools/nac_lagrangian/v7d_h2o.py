"""v7d: seam-identity forensics. In-process FD rot channel
[Mt_rot + 2 gamma_a] : U_rot vs the T2 seam; assembly variants with
each. Also the in-process displaced sweep with FULL record restore.
...
Run: python v7d_h2o.py <input.inp> <dnum.npz>
"""
import os
import sys
import numpy as np
...
def main():
    ...
    r = Runner(input_file=inp, log=inp.replace('.inp', '_v7d.log'))
    r.run()
    mol = r.mol
    ...
    # ---- rotation-block Mt sweeps (frozen-Fock part) ----------------------
    rot_pairs = []
    for ii in range(nocb, noca):
        for jj in range(nocb):
            rot_pairs.append((ii, jj))
    for kk in range(noca, nbf):
        for jj in range(nocb):
            rot_pairs.append((kk, jj))
    for kk in range(noca, nbf):
        for ii in range(nocb, noca):
            rot_pairs.append((kk, ii))
    print(f'rot-block Mt sweeps ({len(rot_pairs)} generators)...', flush=True)
    MTR = {}
    for I in range(nstate):
        for J in range(nstate):
            if I == J:
                continue
            y = ytil[(I, J)]
            sw = np.zeros(len(rot_pairs))
            for ip, (p, q) in enumerate(rot_pairs):
                K = np.zeros((nbf, nbf))
                K[p, q] = 1.0
                K[q, p] = -1.0
                acc = 0.0
    ...
    # ---- displaced sweep for Ux_FD (full record restore after) ------------
    ...
    # ---- FD rot channel + seam identity test ------------------------------
    print('\n===== SEAM IDENTITY: T2 vs [Mt_rot + 2gamma_a] : U_rot(FD) =====')
    T2fd = {}
    for I in range(nstate):
        for J in range(nstate):
            if I == J:
            ...
            for c in range(ncoord):
                ...
                for ip, (p, q) in enumerate(rot_pairs):
                    ...
                    K[p, q] = 1.0
                    K[q, p] = -1.0
                    MK = K * (occ_a[None, :] - occ_a[:, None])
                    MKb = K * (occ_b[None, :] - occ_b[:, None])
                    dDa = C @ (MK * 0) @ C.T
                    g_rot = float(np.sum(Ga * (C @ (K * (occ_a[None, :]
                            - occ_a[:, None])) @ C.T))
                            + np.sum(Gb * (C @ (K * (occ_b[None, :]
                            - occ_b[:, None])) @ C.T)))
                    acc += (MTR[(I, J)][ip] + g_rot
                            + 2.0 * gam[I, J][p, q]) * Ua[p, q]
                t[c] = acc
            T2fd[(I, J)] = t
            cc = float(np.dot(T2[(I, J)], t)) / (
                np.linalg.norm(T2[(I, J)]) * np.linalg.norm(t) + 1e-300)
            print(f'pair ({I+1},{J+1}): |seam|={np.linalg.norm(T2[(I,J)]):.6f} '
                  f'|FDrot|={np.linalg.norm(t):.6f} cos={cc:+.6f} '
                  f'maxdiff={np.abs(T2[(I,J)]-t).max():.3e}', flush=True)

    # ---- assemble (two variants) ------------------------------------------
    for tag, T2use in (('v7c seam', T2), ('FD-rot  ', T2fd)):
        print(f'\n===== v7d ASSEMBLY [{tag}] vs d_num =====')
        dp = np.zeros((ncoord, nstate, nstate))
        for I in range(nstate):
            for J in range(nstate):
                if I == J or T2use[(I, J)] is None:
                    continue
                gsk = np.array([float(np.sum(gam[I, J] * Sk_an[c]))
                                for c in range(ncoord)])
                dp[:, I, J] = (T1[(I, J)] + T2use[(I, J)] + T3s[(I, J)]
                               + T3g[(I, J)] + gsk)
