...
def main():
    ...
    print('sweeping L generators (RESTRICTED: same-space + inter-sym)...', flush=True)
    tt = 1e-4
    ...
    for p in range(nbf):
        for q in range(p + 1):
            ...
            if same_sp or True:
                pass
            # keep: ALL same-space pairs (antisym+sym) and inter-space pairs
            # (sym only -> antisym handled by the z-vector seam). dd/vv are
            # numerically negligible (H2O anatomy) but cheap enough to keep
            # for ss; restrict dd/vv to diagonal-adjacent to bound cost.
            if same_sp:
                b = blk(p)
                if b == 1:                       # ss: keep all (tiny)
                    pq_list.append((p, q, True, True))
                else:                            # dd/vv: sym diagonal only
                    if p == q:
                        pq_list.append((p, q, False, True))
            else:
                pq_list.append((p, q, False, True))   # inter: sym only
    print(f'  {len(pq_list)} generator pairs kept', flush=True)
    for p, q in [(0, 0)]:
        pass
    for (p, q, do_a, do_s) in pq_list:
            K = np.zeros((nbf, nbf))
            if p == q:
                K[p, p] = 1.0
    ...
    np.savez('/bighome/cheolho.choi/nac_audit/ladderA9_eth_data.npz',
             La=La, Ls=Ls,
