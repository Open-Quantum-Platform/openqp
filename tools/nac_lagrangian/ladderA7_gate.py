...
def main():
    ...
    print('\n========= LADDER A7: THREE-TERM FULL CLOSURE =========')
    ...
    for I in range(nstate):
        for J in range(nstate):
            ...
            for k in range(ncoord):
                ...
                Ua = 0.5 * (U - U.T)
                Us = 0.5 * (U + U.T)
                t2[k] = float(np.sum((La[I, J] * Ua + Ls[I, J] * Us) * same)) / gap
                t3[k] = float(np.sum((Ls[I, J] * Us) * inter)) / gap
            print(f'pair ({I+1},{J+1}): |resid|={np.linalg.norm(resid):.6f}  '
                  f'|t1 zB|={np.linalg.norm(t1):.6f}  |t2 same|={np.linalg.norm(t2):.6f}  '
                  f'|t3 interSym|={np.linalg.norm(t3):.6f}')
            best = None
            for s1 in (1, -1):
                for s2 in (1, -1):
                    for s3 in (1, -1):
            ...
            print(f'    best signs ({s1:+d},{s2:+d},{s3:+d}): |v|={np.linalg.norm(v):.6f} '
