"""v7c: v7a with fixed ordering (G-builds last, maxit restored),
the same-space gauge-invariance probe, and per-term vector printouts.
...
Run: python v7c_h2o.py <input.inp> <dnum.npz>
"""
import os
import sys
import numpy as np
...
def main():
    ...
    r = Runner(input_file=inp, log=inp.replace('.inp', '_v7c.log'))
    r.run()
    mol = r.mol
    ...
    # ---- same-space gauge-invariance probe --------------------------------
    probes = [(1, 0), (nocb, nocb - 1) if nocb > 1 else (1, 0),
              (nocb + 1, nocb), (noca + 1, noca), (8, 7), (9, 8)]
    print('gauge-invariance probe (S_swp + 2*gamma_a should be ~0):')
    for I, J in ((0, 1), (1, 2)):
        y = ytil[(I, J)]
        for (p, q) in probes:
            if p >= nbf or q >= nbf:
            ...
            mol.data['OQP::VEC_MO_A'] = W0
            mol.data['OQP::VEC_MO_B'] = Wb0
            print(f'  ({I+1},{J+1}) gen({p},{q}): M~a={acc:+.6f} '
                  f'2g={2*gam[I, J][p, q]:+.6f} sum={acc + 2*gam[I, J][p, q]:+.6f}')
    mol.data['OQP::td_bvec_mo'] = X0_raw
    ...
    for I in range(nstate):
        for J in range(nstate):
            ...
            dp[:, I, J] = (T1[(I, J)] + T2[(I, J)] + T3s[(I, J)]
                           + T3g[(I, J)] + gsk)
            if (I, J) in ((0, 1), (1, 2)):
                print(f'terms ({I+1},{J+1}):')
                for nm, vv in (('T1', T1[(I, J)]), ('T2', T2[(I, J)]),
                               ('T3s', T3s[(I, J)]), ('T3g', T3g[(I, J)]),
                               ('gSk', gsk)):
                    print(f'  {nm:3s}: ' + np.array2string(
                        np.asarray(vv), precision=5, suppress_small=True))
    ...
    np.savez(inp.replace('.inp', '_v7c.npz'), dp=dp, dpa=dpa)
