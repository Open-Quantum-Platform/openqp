"""v7a: v5 assembly with the COMPLETE symmetric channel.
...
Run: python v7a_h2o.py <input.inp> <dnum.npz>
"""
import os
import sys
import numpy as np
...
def main():
    ...
    r = Runner(input_file=inp, log=inp.replace('.inp', '_v7a.log'))
    r.run()
    mol = r.mol
    ...
    # ---- assemble ----------------------------------------------------------
    print('\n===== v7a ASSEMBLY (full sym channel) vs d_num =====')
    ...
    np.savez(inp.replace('.inp', '_v7a.npz'), dp=dp, dpa=dpa)
