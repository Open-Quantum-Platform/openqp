"""v7h: FULL Mt-matrix harvest + savez for offline algebra.
...
def main():
    ...
    r = Runner(input_file=inp, log=inp.replace('.inp', '_v7h.log'))
    r.run()
    mol = r.mol
    ...
    # ---- primary gate + savez ---------------------------------------------
    print('\n===== v7h PRIMARY GATE: (Mt_frozen+Mt_G) o Ux vs ytil.w_ref - T1 =====')
    for I in range(nstate):
        for J in range(nstate):
            if I == J:
                continue
            ...
            print(f'pair ({I+1},{J+1}): |Mt:U|={np.linalg.norm(lhs):.6f} '
                  f'|exact|={np.linalg.norm(rhs):.6f} cos={cc:+.6f} '
    ...
    for (I, J), v in MT.items():
        out[f'MT_{I}{J}'] = v
        out[f'MTG_{I}{J}'] = MTG[(I, J)]
        out[f'T1_{I}{J}'] = T1[(I, J)]
        out[f'T2_{I}{J}'] = T2[(I, J)]
        out[f'T2g_{I}{J}'] = T2g_only[(I, J)]
        out[f'ytil_{I}{J}'] = ytil[(I, J)]
    np.savez(inp.replace('.inp', '_v7h.npz'), **out)
    print('saved v7h npz.', flush=True)
