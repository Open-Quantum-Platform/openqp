...
OUT = '/bighome/alireza/openqp-nac/tools/nac_validation/data_snapshots/p13_esum.npz'
ETA = 1.0e-4     # Fock scaling (exact in principle; small only to stay in float range)
EPS = 1.0e-4     # MO mixing

r = Runner(input_file=INP,
           log='/bighome/alireza/openqp-nac/tools/nac_validation/p13_esum.log')
r.run()
mol = r.mol
...
# ---------------------------------------------------------------- GATE 2
print('\n# GATE 2: closed-form R_esum = [(Gam+Gam^T) f]_qp  vs  Fock-scaling measurement')
