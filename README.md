...
| Multireference PT2 | CASPT2 (SS/MS/XMS), NEVPT2 (uncontracted/strongly contracted), QDPT2 (MRMP2/MCQDPT2/XMCQDPT2) | Native energies; **analytic** nuclear gradients for single-state CASPT2/MRMP2, MCQDPT2 and XMS-CASPT2/XMCQDPT2, central differences for the rest (`[pt2] gradient`); see [`examples/WF_methods`](examples/WF_methods) for variants, controls, and present limits |
...
| --- | --- | --- |
| Analytic gradients | HF, DFT, TDDFT, SF/MRSF-TDDFT, MP2, state-specific CASSCF, CASPT2 (SS/XMS) | State-specific gradients for optimization and dynamics.  The CASPT2 gradient is a full Lagrangian derivative -- amplitude, CI, XMS state-rotation and reference-orbital response -- and carries the real, imaginary and ISA denominator shifts exactly |
| Numerical gradients | SA-CASSCF, multi-set MS-CASPT2, NEVPT2, SC-NEVPT2 | Central differences of converged multireference energies for `grad`, `optimize`, `ts`, `mep`, and `irc`; also the automatic fallback for any PT2 option combination outside the analytic gradient's scope |
