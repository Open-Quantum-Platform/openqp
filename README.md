...
| --- | --- | --- | --- |
| Analytic nuclear gradients | MRSF/SF/TDDFT, HF/DFT, RHF-based MP2, state-specific CASSCF, and strongly contracted NEVPT2 | State-specific derivatives for `grad`, optimization, reaction paths, and dynamics; SC-NEVPT2 solves the coupled CASSCF orbital/CI response and the semicanonical-basis response, at a cost independent of the number of nuclei | [Overview](https://open-quantum-platform.github.io/openqp-docs/capabilities/) |
| Numerical nuclear gradients | SA-CASSCF and the CASPT2/NEVPT2/QDPT2 variants without an analytic derivative | Central differences of converged energies for `grad`, `optimize`, `ts`, `mep`, and `irc`; also the fallback whenever the analytic SC-NEVPT2 route does not apply | [Examples](examples/WF_methods) |
