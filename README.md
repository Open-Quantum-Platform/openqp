...
| --- | --- | --- | --- |
| Analytic nuclear gradients | MRSF/SF/TDDFT, HF/DFT, RHF-based MP2, state-specific CASSCF and SA-CASSCF, and strongly contracted NEVPT2 | State-specific derivatives for `grad`, optimization, and reaction paths; `runtype=namd` dynamics is MRSF-TDDFT only, and the CASSCF/SA-CASSCF and PT2 routes accept grad/optimize/ts/mep/irc; SA-CASSCF additionally differentiates the weighted objective, which is not a state and is selected by `[casscf] gradient_state` rather than by `[properties] grad`; SC-NEVPT2 solves the coupled CASSCF orbital/CI response and the semicanonical-basis response, at a cost independent of the number of nuclei | [Overview](https://open-quantum-platform.github.io/openqp-docs/capabilities/) |
| Numerical nuclear gradients | The legacy `method=casscf` plus `[state_average]` spelling, and the CASPT2/NEVPT2/QDPT2 variants without an analytic derivative | Central differences of converged energies for `grad`, `optimize`, `ts`, `mep`, and `irc`; also the fallback whenever the analytic SC-NEVPT2 route does not apply | [Examples](examples/WF_methods) |
