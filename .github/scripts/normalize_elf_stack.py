#!/usr/bin/env python3
"""Reject executable-stack requests in packaged OpenQP ELF libraries."""
...
def require_non_executable_stack(path: Path) -> None:
    flags = gnu_stack_flags(path.read_bytes())
    if flags & PF_X:
        raise RuntimeError(
            f"packaged ELF requests an executable stack: {path}; "
            "compile trampoline users out of the process stack before packaging"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--openqp-package",
        action="store_true",
        help="verify every ELF shared library in the installed oqp/lib directory",
    )
    ...
    for path in paths:
        ...
        if resolved in seen:
            continue
        seen.add(resolved)
        require_non_executable_stack(resolved)
        print(f"PT_GNU_STACK non-executable: {resolved}")
