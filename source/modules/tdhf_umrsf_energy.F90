module tdhf_umrsf_energy_mod

  implicit none

  character(len=*), parameter :: module_name = "tdhf_umrsf_energy_mod"
...
contains
  ...
  subroutine tdhf_umrsf_energy_C(c_handle) bind(C, name="tdhf_umrsf_energy")
    ...
    call tdhf_umrsf_energy(inf)
  end subroutine tdhf_umrsf_energy_C

  subroutine tdhf_umrsf_energy(infos)
    ...
    character(len=*), parameter :: subroutine_name = "tdhf_umrsf_energy"
  ...
  !
    call print_module_info('U-MRSF_TDHF_Energy','Computing Energy of UHF-based MRSF-TDDFT')
...
!   infos%tddft%debug_mode = .True.
    ...
    if (mol_mult/=3) call show_message('MRSF-TDDFT are available for UHF ref.&
        &with ONLY triplet multiplicity(mult=3)',with_abort)

    scf_type = infos%control%scftype
    if (scf_type/=2) call show_message('U-MRSF requires UHF reference (SCFTYPE=2).',with_abort)
...
!     No space left for new vectors, exit
  ...
  end subroutine tdhf_umrsf_energy


end module tdhf_umrsf_energy_mod