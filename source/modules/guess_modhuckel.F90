module guess_modhuckel_mod

  implicit none

  character(len=*), parameter :: module_name = "guess_modhuckel_mod"
...
contains
  ...
  subroutine guess_modhuckel_C(c_handle) bind(C, name="guess_modhuckel")
    ...
    call guess_modhuckel(inf)
  end subroutine guess_modhuckel_C

  subroutine guess_modhuckel(infos)
    ...
    character(len=*), parameter :: subroutine_name = "guess_modhuckel"
  ...
  ! Calculate modified Huckel MO
    if (pe%rank == root) then
      call huckel_guess(Smat, MO_A, infos, basis, huckel_basis, modified=.true.)
    endif
  !  For ROHF/UHF
  ...
  ! ROHF/UHF
    ...
    close(iw)
  end subroutine guess_modhuckel

end module guess_modhuckel_mod
