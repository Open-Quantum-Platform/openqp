...
contains
  ...
  subroutine tdhf_mrsf_ekt(infos, electron_affinity)
    ...
    ! Z-vector.  OQP_td_p stores spin-resolved alpha/beta difference-density
    ! corrections in the AO basis.  Keep the EKT hole/particle metrics
    ! spin-resolved: summing alpha and beta makes closed-shell occupied
    ! orbitals carry occupation near two, breaks the I-P particle metric, and
    ! collapses the H2O relaxed-density IP comparison to the wrong low-energy
    ! root.
    density_alpha_ao = dmat_a + td_p(:,1)
    density_beta_ao = dmat_b + td_p(:,2)
    call orthogonal_transform_sym(nbf, nbf, density_alpha_ao, mo_a, nbf, dens_pack)
    call unpack_matrix(dens_pack, density_alpha_mo, nbf, 'U')
    call orthogonal_transform_sym(nbf, nbf, density_beta_ao, mo_a, nbf, dens_pack)
    call unpack_matrix(dens_pack, density_beta_mo, nbf, 'U')
