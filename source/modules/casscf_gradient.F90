...
!###############################################################################
  ...
  function casscf_gradient(infos, iopt, dopt, weights, roots, info) &
    ...
    ! A full active space has no non-redundant orbital rotations.  The energy
    ! path correctly falls back to its already-converged Python result, while
    ! the nuclear gradient still needs the CI/RDM context and is exactly
    ! stationary with |g_orb| = 0 by construction.
    call cas_context_init(infos, iopt, dopt, weights, roots, ctx, cbuf, status, &
                          allow_zero_rotations=.true.)
    ...
    allocate(gorb(0:max(ctx%npar, 1)-1), stat=ierr)
    if (ierr /= 0) then
    ...
    if (ctx%npar > 0) then
      info(CAS_G_GNORM) = sqrt(sum(gorb(0:ctx%npar-1) * gorb(0:ctx%npar-1)))
    else
      info(CAS_G_GNORM) = 0.0_dp
    end if
