module tdhf_mrsf_hessian_amplitude_mod
  ...
  use tdhf_hessian_response_mod, only: solve_mrsf_tda_response_matrix_free
...
!###############################################################################
  ...
  subroutine solve_mrsf_tda_amplitude_derivatives(infos,int2_driver,mo_a,mo_b, &
    ...
    real(kind=dp), allocatable :: transform(:,:),x_physical(:),dax_physical(:,:), &
      dx_physical(:,:)
    real(kind=dp) :: solve_tolerance
    integer :: nbf,nocca,noccb,packed,physical,ncoord,solve_iterations, &
      solve_restart,space_status
