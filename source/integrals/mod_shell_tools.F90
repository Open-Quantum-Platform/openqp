...
!dir$ attributes inline :: bas_set_indices
    ...
    shinfo%shid  = shid
    shinfo%atid  = basis%katom(shid)
    shinfo%ig1   = basis%kstart(shid)
    shinfo%ig2   = basis%kstart(shid)+basis%kng(shid)-1
    shinfo%ang   = basis%ktype(shid)
    shinfo%locao = basis%kloc(shid)
    shinfo%nao   = basis%kmax(shid)-basis%kmin(shid)+1
