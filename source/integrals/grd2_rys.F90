module grd2_rys
    ...
    integer, private :: iii
    integer, parameter :: GRD2_RYS_MXANG = 7
    integer, parameter :: GRD2_RYS_MXCART = GRD2_RYS_MXANG*(GRD2_RYS_MXANG+1)/2
    integer, parameter :: ijkln(GRD2_RYS_MXANG) = [ (iii*(iii+1)/2, iii = 1, GRD2_RYS_MXANG) ]
    integer, parameter :: MAXCONTR = 120
   !< power of X in Cartesian Gaussian basis functions
    integer, parameter :: &
   ijklx(GRD2_RYS_MXCART,GRD2_RYS_MXANG) = reshape([ &
      [0,                                                               (0, iii = ijkln(1)+1, GRD2_RYS_MXCART)], &
      [1, 0, 0,                                                         (0, iii = ijkln(2)+1, GRD2_RYS_MXCART)], &
      [2, 0, 0, 1, 1, 0,                                                (0, iii = ijkln(3)+1, GRD2_RYS_MXCART)], &
      [3, 0, 0, 2, 2, 1, 0, 1, 0, 1,                                    (0, iii = ijkln(4)+1, GRD2_RYS_MXCART)], &
      [4, 0, 0, 3, 3, 1, 0, 1, 0, 2, 2, 0, 2, 1, 1,                     (0, iii = ijkln(5)+1, GRD2_RYS_MXCART)], &
      [5, 0, 0, 4, 4, 1, 0, 1, 0, 3, 3, 2, 0, 2, 0, 3, 1, 1, 2, 2, 1,   (0, iii = ijkln(6)+1, GRD2_RYS_MXCART)], &
      [6, 0, 0, 5, 5, 1, 0, 1, 0, 4, 4, 2, 0, 2, 0, 4, 1, 1, 3, 3, 0, 3, 3, 2, 1, 2, 1, 2]  &
      ], shape(ijklx))

   !< power of Y in Cartesian Gaussian basis functions
    integer, parameter :: &
   ijkly(GRD2_RYS_MXCART,GRD2_RYS_MXANG) = reshape([ &
      [0,                                                              (0, iii = ijkln(1)+1, GRD2_RYS_MXCART)], &
      [0, 1, 0,                                                        (0, iii = ijkln(2)+1, GRD2_RYS_MXCART)], &
      [0, 2, 0, 1, 0, 1,                                               (0, iii = ijkln(3)+1, GRD2_RYS_MXCART)], &
      [0, 3, 0, 1, 0, 2, 2, 0, 1, 1,                                   (0, iii = ijkln(4)+1, GRD2_RYS_MXCART)], &
      [0, 4, 0, 1, 0, 3, 3, 0, 1, 2, 0, 2, 1, 2, 1,                    (0, iii = ijkln(5)+1, GRD2_RYS_MXCART)], &
      [0, 5, 0, 1, 0, 4, 4, 0, 1, 2, 0, 3, 3, 0, 2, 1, 3, 1, 2, 1, 2,  (0, iii = ijkln(6)+1, GRD2_RYS_MXCART)], &
      [0, 6, 0, 1, 0, 5, 5, 0, 1, 2, 0, 4, 4, 0, 2, 1, 4, 1, 3, 0, 3, 2, 1, 3, 3, 1, 2, 2] &
      ], shape(ijkly))

   !< power of Z in Cartesian Gaussian basis functions
    integer, parameter :: &
   ijklz(GRD2_RYS_MXCART,GRD2_RYS_MXANG) = reshape([ &
      [0,                                                              (0, iii = ijkln(1)+1, GRD2_RYS_MXCART)], &
      [0, 0, 1,                                                        (0, iii = ijkln(2)+1, GRD2_RYS_MXCART)], &
      [0, 0, 2, 0, 1, 1,                                               (0, iii = ijkln(3)+1, GRD2_RYS_MXCART)], &
      [0, 0, 3, 0, 1, 0, 1, 2, 2, 1,                                   (0, iii = ijkln(4)+1, GRD2_RYS_MXCART)], &
      [0, 0, 4, 0, 1, 0, 1, 3, 3, 0, 2, 2, 1, 1, 2,                    (0, iii = ijkln(5)+1, GRD2_RYS_MXCART)], &
      [0, 0, 5, 0, 1, 0, 1, 4, 4, 0, 2, 0, 2, 3, 3, 1, 1, 3, 1, 2, 2,  (0, iii = ijkln(6)+1, GRD2_RYS_MXCART)], &
    ...
    type grd2_int_data_t
      ...
      integer :: ijklxyz(4,GRD2_RYS_MXCART,4)
      real(kind=dp) :: fd(3,4)
      real(kind=dp) :: dtol
      real(kind=dp) :: dabcut
      contains
...
contains
  ...
  subroutine gdat_set_ids(gdat, basis, i, j, k, l)
    ...
    integer :: id(4), shtyp(4), flips(4), tmp(2), same

    ! Permute shells so L_i < L_j, L_k < L_l, L_i < L_j
    flips = [1,2,3,4]
    id = [i,j,k,l]
    shtyp = basis%ktype([i,j,k,l])
    if (shtyp(1) > shtyp(2)) flips(1:2) = [2,1]
    if (shtyp(3) > shtyp(4)) flips(3:4) = [4,3]
    if (max(shtyp(1),shtyp(2)) > max(shtyp(3),shtyp(4))) then
      tmp = flips(1:2)
      flips(1:2) = flips(3:4)
      flips(3:4) = tmp
    end if
...
!   Prepare shell block
    ...
    nimax = gdat%shtyp(1) + gdat%der(1)
    njmax = gdat%shtyp(2) + gdat%der(2)
    nkmax = gdat%shtyp(3) + gdat%der(3)
    nlmax = gdat%shtyp(4) + gdat%der(4)

    nmax = gdat%shtyp(1)+gdat%shtyp(2)-1 + min(gdat%der(1)+gdat%der(2),gdat%nder)
    mmax = gdat%shtyp(3)+gdat%shtyp(4)-1 + min(gdat%der(3)+gdat%der(4),gdat%nder)
...
!   Set number of quadrature points
    gdat%nroots = (sum(gdat%shtyp)-2 + gdat%nder )/2
...
!   Prepare indices for pairs of (i,j) functions
  ...
  subroutine prepare_xyz_ids(gdat)
    ...
    nj = gdat%shtyp(2) + gdat%der(2)
    nk = gdat%shtyp(3) + gdat%der(3)
    nl = gdat%shtyp(4) + gdat%der(4)
    njkl = nl*nk*nj
    do i = 1, gdat%nbf(1)
      gdat%ijklxyz(1,i,1) = ijklx(i,gdat%shtyp(1))*njkl
      gdat%ijklxyz(2,i,1) = ijkly(i,gdat%shtyp(1))*njkl
      gdat%ijklxyz(3,i,1) = ijklz(i,gdat%shtyp(1))*njkl
    end do

    nkl = nl*nk
    do i = 1, gdat%nbf(2)
      gdat%ijklxyz(1,i,2) = ijklx(i,gdat%shtyp(2))*nkl
      gdat%ijklxyz(2,i,2) = ijkly(i,gdat%shtyp(2))*nkl
      gdat%ijklxyz(3,i,2) = ijklz(i,gdat%shtyp(2))*nkl
    end do

!   Prepare indices for pairs of (k,l) functions
    do i = 1, gdat%nbf(3)
      gdat%ijklxyz(1,i,3) = ijklx(i,gdat%shtyp(3))*nl
      gdat%ijklxyz(2,i,3) = ijkly(i,gdat%shtyp(3))*nl
      gdat%ijklxyz(3,i,3) = ijklz(i,gdat%shtyp(3))*nl
    end do

    do i = 1, gdat%nbf(4)
      gdat%ijklxyz(1,i,4) = ijklx(i,gdat%shtyp(4))+1
      gdat%ijklxyz(2,i,4) = ijkly(i,gdat%shtyp(4))+1
      gdat%ijklxyz(3,i,4) = ijklz(i,gdat%shtyp(4))+1
    end do
  end subroutine
