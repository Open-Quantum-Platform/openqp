...
!       J shell
        DO jj = 1, basis%nshell
            ...
            CALL comp_overlap_der1(cntp, dens(basis%kloc(ii):, basis%kloc(jj):), de_atom)
        END DO
...
!       J shell
        DO jj = 1, basis%nshell
            ...
            CALL comp_kinetic_der1(cntp, dens(basis%kloc(ii):, basis%kloc(jj):), de_atom)
...
!           Nuclear attraction derivative
            DO ic = 1, nat
                CALL comp_coulomb_der1(cntp, coord(:,ic), -zq(ic), dens(basis%kloc(ii):, basis%kloc(jj):), dernuc)
...
!                    .AND. (znuc**2 < scrthr**2 * sum((cxyz-shi%r)**2)) ) CYCLE
                ...
                CALL comp_coulomb_der1(cntp, cxyz, znuc, dens(basis%kloc(ii):, basis%kloc(jj):), dernuc)

                ! Ewald screening
                IF (present(alpha)) THEN
                    CALL comp_ewaldlr_der1(cntp, cxyz, -znuc, dens(basis%kloc(ii):, basis%kloc(jj):), alpha, dernuc)
                END IF
...
atoms:      DO ic = 1, nat
                ...
                CALL comp_coulomb_helfeyder1(cntp, coord(:,ic), -zq(ic), dens(basis%kloc(ii):, basis%kloc(jj):), dernuc)
