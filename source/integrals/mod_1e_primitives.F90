...
!  NBFS(7) = [((iii+1)*(iii)/2, iii = 1, 7)], &
 ...
 integer, parameter :: MAX_ANG = 6+1
 integer, parameter :: MAX_ANG_PAD = 8
...
!dir$ attributes inline :: comp_kin_ovl_int1_prim
    ...
    real(real64) :: xyzovl(max_ang+2,max_ang,3)
    real(real64) :: xyzkin(max_ang_pad,max_ang,3)
...
!dir$ attributes inline :: comp_coulomb_int1_prim
    ...
    REAL(REAL64) :: dum, dij
    real(real64) :: xyzin(2*max_ang, max_ang+1,3,max_nroots)
...
!dir$ attributes forceinline :: comp_fmoesp_int1_prim
    ...
    INTEGER, INTENT(IN)   :: id
    REAL(REAL64),   INTENT(IN)    :: c(3), znuc
    REAL(REAL64),   INTENT(IN)    :: fracv, fracesp
    REAL(REAL64), CONTIGUOUS,   INTENT(INOUT) :: vblk(:), zblk(:)
...
!dir$ attributes inline :: int1_lz_prim
    ...
    REAL(REAL64) :: dum2
    real(real64) :: xyzovl(max_ang+2,max_ang,3)
    real(real64) :: xyzlz(max_ang_pad,max_ang,2)
!dir$ assume_aligned zblk : 64
!dir$ assume_aligned xyzovl : 64
...
!dir$ attributes inline :: comp_kin_ovl_int1_prim
    ...
    REAL(REAL64) :: tmp(10)
    real(real64) :: xyzmom(3,0:3,max_ang,max_ang)
!dir$ assume_aligned sblk : 64
!dir$ assume_aligned tblk : 64
!dir$ assume_aligned xyzkin : 64
!dir$ assume_aligned xyzovl : 64
...
!dir$ attributes inline :: comp_overlap_der1
    ...
    real(real64) :: ovl_int(max_ang,max_ang+3,3)
    real(real64) :: ovl_der(max_ang,max_ang,3)
...
!dir$ attributes inline :: comp_kinetic_der1
    ...
    real(real64) :: kin_int(max_ang,max_ang+1,3)
    real(real64) :: kin_der(max_ang,max_ang,3)
...
!dir$ attributes inline :: comp_coulomb_der1
    ...
    real(real64) :: xyzin(2*max_ang, max_ang+1,3,max_nroots)
    real(real64) :: dxyzc(max_ang_pad,max_ang,3,max_nroots)
!dir$ assume_aligned xyzin : 64
...
!dir$ attributes forceinline :: overlap_xyz
    ...
    REAL(REAL64), CONTIGUOUS, INTENT(OUT)  :: xyzovl(:,:,:)
...
!dir$ attributes forceinline :: kinetic_xyz_i
    REAL(REAL64), CONTIGUOUS, INTENT(OUT) :: xyzt(:,:,:)
    REAL(REAL64), CONTIGUOUS, INTENT(IN)  :: xyzs(:,:,:)
    ...
    xyzt(1:nj,2,:) = (xyzs(1:nj,2,:)*3.0D0 - 2*ai*xyzs(1:nj,4,:))*ai

    IF (ni==2) RETURN

    DO i = 3, ni
      fact1 = 2*i-1
      fact2 = real((i-1)*(i-2)/2,REAL64)
...
!dir$ attributes forceinline :: kinetic_xyz_j
    ...
    xyzt(2,1:ni,:) = (xyzs(2,1:ni,:)*3.0D0 - 2*aj*xyzs(4,1:ni,:))*aj

    IF (nj==2) RETURN

    DO j = 3, nj
      fact1 = 2*j-1
      fact2 = real((j-1)*(j-2)/2,REAL64)
...
!dir$ attributes forceinline :: overlap_xyz
    ...
    INTEGER, INTENT(IN)   :: li, lj
    integer, intent(in) :: mxmom
    REAL(REAL64), CONTIGUOUS, INTENT(OUT)  :: xyzints(:,0:,:,:)
...
!dir$ attributes forceinline :: der_coul_xyz
    REAL(REAL64), INTENT(IN) ::  ai
    REAL(REAL64), CONTIGUOUS, INTENT(IN) ::  xyzin(:,:,:,:)
    REAL(REAL64), CONTIGUOUS, INTENT(OUT) :: dxyzdi(:,:,:,:)
...
!dir$ attributes forceinline :: der_helfey_xyz
    ...
    REAL(REAL64), CONTIGUOUS, INTENT(IN) ::  xyzin(:,:,:,:)
    REAL(REAL64), CONTIGUOUS, INTENT(OUT) :: dxyzdc(:,:,:,:)
    INTEGER, INTENT(IN) :: lit, ljt, nroots
!dir$ assume_aligned xyzin : 64
...
!dir$ attributes forceinline :: der_kinovl_xyz
    ...
    REAL(REAL64), CONTIGUOUS, INTENT(OUT) :: dxyz(:,:,:)
