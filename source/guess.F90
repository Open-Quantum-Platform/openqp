...
!>          harmonic transformation (contaminants now dropped).
!>          This is used for occupied atoms, so does not need to
!>          go beyond f orbitals for the known periodic table.
!
...
!
!     ----- GENERATE TRANSFORMATION TO SPHERICAL HARMONIC BASIS -----
!     This should only be called by a s,p,d,f basis set,
!     such as the MINI basis used in the Huckel guess.
!     The transformation -W- returned is orthogonalizing within shells,
!     namely W-DAGGER*S*W = I, for all S,P,D,F diagonal blocks.
!     On any given atom, there will be no overlap between S,P,D,F
!     orbitals, but intra-atomic SS, PP, DD, FF overlaps and of course
!     overlaps with other atoms are still present.
!     Note: -W- is rectangular in shape, L1CO by L0CO, where -L0CO-
!     returns the number of pure spherical harmonic AOs found.
!
      w = 0
      sph = 1
      ncont = 0
      notsp = .false.
      do n = 1, basis%nshell
         notsp = notsp .or. (basis%ktype(n) > 2)
!
!        S,P,L shell is already spherical harmonics.
         if (basis%ktype(n) <= 2) then
            cart = basis%kloc(n)
            ndim = basis%kmax(n) - basis%kmin(n) + 1
            do l = 1, ndim
