...
!> How many irreducible representations would receive NO initial trial
!> vector if the energy-ordered selection had dimension nvec.
!>
!> Zero means the guess already reaches every block of A and nothing needs to
!> change.  A positive value is the number of extra trial vectors required to
!> stop the corresponding roots from being silently absent from the spectrum.
!> Returns 0 when no per-pair irrep table is staged (C1, mixed orbitals, or
!> symmetry detection disabled), where the question does not arise.
  integer function mrsf_uncovered_irreps(infos, ea, eb, nvec) result(nunc)
    ...
    integer(4) :: ta_status
    real(kind=dp), allocatable :: xm(:), xtmp(:)
    integer, allocatable :: itmp(:)
    logical, allocatable :: seen(:)
    integer :: nbf, nocca, lr1, lr2, mrst, xvec_dim, i, j, k, ij, nirr, ir
    ...
    xvec_dim = nocca*(nbf - (nocca-2))
    if (size(pair_irrep) /= xvec_dim) return
    nirr = int(maxval(pair_irrep))
    if (nirr < 2) return                 ! single block: nothing can be missed

    allocate(xm(xvec_dim), itmp(nvec), xtmp(nvec), seen(nirr))
    ! same diagonal estimate and masking as mrinivec
    ij = 0
    do j = lr1, nbf
      do i = 1, lr2
        ij = ij + 1
    ...
    do k = 1, nvec
      if (itmp(k) >= 1 .and. itmp(k) <= xvec_dim) then
        ...
        if (ir >= 1 .and. ir <= nirr) seen(ir) = .true.
      end if
    end do
    do ir = 1, nirr
      if (seen(ir)) cycle
      ! only count irreps that actually own a live pair
      do i = 1, xvec_dim
        if (int(pair_irrep(i)) == ir .and. xm(i) < 1.0d90) then
          nunc = nunc + 1
          exit
        end if
      end do
    end do
    deallocate(xm, itmp, xtmp, seen)
  end function mrsf_uncovered_irreps
