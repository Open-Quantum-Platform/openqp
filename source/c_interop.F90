...
!--------------------------------------------------------------------------------
  ...
  function oqp_get_basis(c_handle, nsh, nprim, nbf, bt, at, cdeg, ex, cc) result(ret) bind(C, name='oqp_get_basis')
    use basis_tools, only: basis_set
    type(oqp_handle_t) :: c_handle
    integer(c_int64_t) :: nsh, nprim, nbf
    integer(c_int64_t) :: ret
    type(c_ptr), intent(out) :: bt, at, cdeg, ex, cc
...
#define ADDRESSOF(a,b) if(allocated(a))then;b=c_loc(a);else;return;endif
    ADDRESSOF(bas%ex,    ex)
    ADDRESSOF(bas%cc,    cc)
    ADDRESSOF(bas%ktype, bt)
    ADDRESSOF(bas%katom, at)
    ADDRESSOF(bas%kng,   cdeg)
#undef ADDRESSOF
