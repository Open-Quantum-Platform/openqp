...
class WindowsMsys2PackagingTests(unittest.TestCase):
    ...
    def test_pkgbuild_defines_real_msys2_package(self):
        ...
        self.assertIn("OQP_MSYS2_EXTERNALS_ROOT", pkgbuild)
        self.assertIn('$(cygpath -u "${RUNNER_TEMP}")/oqp-ext', pkgbuild)
        self.assertIn('_external_root="$(cygpath -m "${_externals_root}")"', pkgbuild)
        ...
        self.assertIn("OQP_MSYS2_EXTERNALS_ROOT", builder)
        self.assertIn('$(cygpath -u "$RUNNER_TEMP")/oqp-ext', builder)
