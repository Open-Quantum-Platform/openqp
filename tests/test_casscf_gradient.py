...
def test_state_averaged_casscf_gradient_is_refused_by_preflight():
    """A state-averaged gradient request is rejected before anything runs.

    Only the weighted sum ``sum_I w_I E_I`` is stationary against orbital
    rotation, so an individual averaged state needs a Lagrangian/Z-vector
    response.  Handing it the state-specific expression would be wrong by
    exactly the term that is missing, and wrong quietly -- so the refusal is a
    preflight error naming the reason, not a late failure.
    """
    sa = {"enabled": True, "nstate": 2}
    report = _preflight("sa-casscf", "grad", state_average=sa)
    assert not report.ok
    text = report.to_text()
    assert "Z-vector" in text or "state-specific" in text

    # ...and the same method with runtype=energy is accepted: it is the
    # gradient that is missing, not the method.
    assert _preflight("sa-casscf", "energy", state_average=sa).ok


def test_state_averaged_casscf_gradient_is_refused_by_the_library(tmp_path):
    """The library guard refuses too, for a caller that bypasses preflight.

    ``Gradient`` is reachable directly from the Python API and from the
    optimizer drivers, so the check cannot live only in the input checker.
    """
    if not _backend_available():
        pytest.skip("native OQP backend not built; build liboqp to run this test")
