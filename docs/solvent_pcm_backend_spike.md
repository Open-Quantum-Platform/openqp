...
- public unweighted electronic MEP wrapper `electrostatic_potential_unweighted` in `source/integrals/int1.F90`
...
Result: both `oqp_ddx_link_smoke` and `oqp_ddx_adapter_smoke` passed. The adapter smoke now covers the high-level `ddx_ddrun` point-charge lifecycle, an explicit `ddx_pcm_setup`/forward/adjoint solve path with host-supplied `psi` and `phi_cav`, and a reaction-field handoff smoke that copies `cavity_xyz[3*ncav]` plus `q_cav[ncav]` for the future `external_charge_potential` seam. Source inspection shows `ddx_get_xi` projects ddPCM `state%q` to cavity points, so the smoke result labels this as `q_cav_norm` as well as the legacy `xi_norm`. A finite-difference perturbation of `phi_cav[0]` shows `dE/dphi_cav[0] ≈ -0.5*q_cav[0]`; direct `q_cav` has the wrong sign/scale for this derivative in the smoke case. The next OpenQP step is to build a guarded prototype AO matrix from `chg = -0.5*q_cav`, then cross-check against an independent reference before enabling runtime PCM.
