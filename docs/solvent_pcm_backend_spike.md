...
Result: both `oqp_ddx_link_smoke` and `oqp_ddx_adapter_smoke` passed. The adapter smoke now covers both the high-level `ddx_ddrun` point-charge lifecycle and an explicit `ddx_pcm_setup`/forward/adjoint solve path with host-supplied `psi` and `phi_cav`. Source inspection shows `ddx_get_xi` projects ddPCM `state%q` to cavity points, so the smoke result labels this as `q_cav_norm` as well as the legacy `xi_norm`. The next OpenQP step is to replace the smoke point-source `psi`/`phi_cav` with AO-density/nuclear-potential quantities, then validate whether this cavity-projected `state%q` is the physically correct charge-like vector for `external_charge_potential` before adding it to `calc_jk_xc`.
