...
Result: both `oqp_ddx_link_smoke` and `oqp_ddx_adapter_smoke` passed. The adapter smoke now covers both the high-level `ddx_ddrun` point-charge lifecycle and an explicit `ddx_pcm_setup`/forward/adjoint solve path with host-supplied `psi` and `phi_cav`. The next OpenQP step is to replace the smoke point-source `psi`/`phi_cav` with AO-density/nuclear-potential quantities, then identify or expose the physically correct ddPCM Fock/Kohn-Sham reaction-field quantity (`q`/`qgrid` or equivalent) before adding it to `calc_jk_xc`.
