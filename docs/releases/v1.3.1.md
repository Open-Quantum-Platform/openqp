...
- **Line endings are pinned to LF via `.gitattributes`.** Windows checks out
  with `core.autocrlf` enabled by default, which rewrote LF to CRLF and made
  the Windows wheels ship license text that no longer matched the tagged
  source. The repository was already entirely LF, so no tracked file changed --
  only the guarantee that a checkout is byte-identical on every machine
  ([#364](https://github.com/Open-Quantum-Platform/openqp/pull/364)).
