...
- **In progress — one-electron `der2` integral terms** (`sum P * d2h - sum W * d2S`):
  ...
  - **In progress — nuclear-attraction second derivatives.** The 1/r operator uses Rys quadrature and depends on three centers: bra A, ket B, and the nuclear charge C. The design avoids any second-derivative Rys-root machinery by reducing everything to the three basis-center blocks:
    ...
    **Corrected plan.** Obtain `d2/dB2` from the bra second derivative of the *swapped* shell pair (bra<->ket), which reuses the validated bra path. Obtain the mixed `d2/dA dB` from the bra derivative of the Hellmann-Feynman charge-center derivative combined with translational invariance: `d2/dA dB = -(d2/dA2 + d2/dA dC)`, where `d2/dA dC` is the bra derivative of the existing `der_helfey_xyz`/`DQGaussRys` charge term. This keeps every piece on a production-validated derivative path. Then build `comp_coulomb_der2` (AA/AB/BB blocks), the `hess_en` driver (scatter over bra/ket/nucleus via translational invariance), and FD-validate against `grad_en_hellman_feynman` + `grad_en_pulay`.
