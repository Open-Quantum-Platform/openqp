...
- **Done so far (the complete 1e RHS):** `grd1::der_overlap_matrix` (`dS/dR`), `der_kinetic_matrix` (`dT/dR`), and `der_nucattr_matrix` (`dV/dR`) build the AO first-derivative matrices (nbf,nbf,3,natom) for every nuclear coordinate, from non-contracting `comp_*_der1_block` primitives. Overlap/kinetic scatter bra(+)/ket(-) by translational invariance; nuclear attraction scatters bra->A, charge->C (Hellmann-Feynman), and ket = -(bra+charge). All three are validated in `hess1_selftest` by contracting with the normalized density and matching the production gradients (`grad_ee_overlap`, `grad_ee_kinetic`, `grad_en_pulay`+`grad_en_hellman_feynman`) to ~1e-14. They use only OpenQP's own Rys first-derivative code (no libint). Remaining for the RHS: the two-electron `dJ/dx[P]`/`dK/dx[P]` response (the libint deriv-1 Fock build), then the PCG solve reusing the Z-vector machinery.
