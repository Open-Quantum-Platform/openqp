...
- **In progress — one-electron `der2` integral terms** (`sum P * d2h - sum W * d2S`):
  ...
  - **In progress — nuclear-attraction second derivatives.** The 1/r operator uses Rys quadrature and depends on three centers: bra A, ket B, and the nuclear charge C. The design avoids any second-derivative Rys-root machinery by reducing everything to the three basis-center blocks:
    ...
    **Remaining:** ket-center first/second-derivative recursions and the mixed `d2/dA dB` term; a `comp_coulomb_der2` primitive returning the AA/AB/BB blocks; a `hess_en` driver that scatters them (and the C/mixed blocks via translational invariance) over the bra atom, ket atom, and each nucleus into the `(3N,3N)` Hessian; and FD validation against `grad_en_hellman_feynman` + `grad_en_pulay` (extending the `hess1_selftest` harness).
