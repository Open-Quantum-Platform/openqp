...
- **Done so far:** `grd1::der_overlap_matrix` builds the AO `dS/dR` matrices (nbf,nbf,3,natom) from `comp_overlap_der1_block`, scattering bra/ket via translational invariance. Validated in `hess1_selftest` by contracting with the normalized density and matching `grad_ee_overlap` to ~1e-15. The kinetic (`dT/dR`) matrix follows the same pattern; the nuclear-attraction (`dV/dR`) matrix adds the charge-center (Hellmann-Feynman) contribution; the two-electron `dJ/dx[P]`/`dK/dx[P]` response is the libint deriv-1 Fock-build piece.
