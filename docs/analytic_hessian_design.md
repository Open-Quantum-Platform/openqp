...
- **In progress — one-electron `der2` integral terms** (`sum P * d2h - sum W * d2S`):
  ...
  - **Next — nuclear-attraction second derivatives** (`comp_coulomb_der2`): more involved than overlap/kinetic because the 1/r operator uses Rys quadrature and introduces the (third) nuclear-charge center, so the simple two-center translational-invariance shortcut does not apply directly.
