...
- **In progress — one-electron `der2` integral terms** (`sum P * d2h - sum W * d2S`):
  ...
  - **In progress — nuclear-attraction second derivatives.** The 1/r operator uses Rys quadrature and depends on three centers: bra A, ket B, and the nuclear charge C. The design avoids any second-derivative Rys-root machinery by reducing everything to the three basis-center blocks:
    ...
    **Open issue.** With the FD harness extended to nuclear attraction (`hess1_selftest`, "nucattr WIP" line), the overlap/kinetic terms still validate, but `hess_en` disagrees with FD (H2/STO-3G: e.g. zz analytic `-0.208` vs FD `-0.488`). The `p_AA` (bra second derivative) and the swapped-pair `p_BB` are on the validated bra path; the discrepancy is localized to the HF-based `p_AC` magnitude (a sign flip makes it worse, ruling out a pure sign error). The driver, primitive, and DQ extension are committed as WIP, clearly marked and not wired into any production path (`hf_hessian` stays guarded). Next: resolve the `p_AC` normalization/composition against a direct finite-difference of the Hellmann-Feynman gradient with respect to the bra-center position.
