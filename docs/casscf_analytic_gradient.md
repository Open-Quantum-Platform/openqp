...
`irc`, `neb`) with `method=casscf` computes the nuclear gradient analytically.
...
- **`method=sa-casscf` / `[state_average] enabled=true`.** A state-averaged run
  ...
  solve. Neither is implemented, and the second must not be claimed without it.
...
- **A `[properties] grad` selector naming a state other than the optimized
  root.** State-specific CASSCF publishes one state.
